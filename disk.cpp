...
void FCFS(vector<int> requests, int head) { calculateSeekTime("FCFS", requests, head); }
...
// Find the closest unvisited request for (int j = 0; j < requests.size(); j++) {
...
if (req <head) left.push_back(req); else right.push_back(req);
}

sort(left.begin(), left.end()); sort(right.begin(), right.end());

if (direction) { sequence.insert(sequence.end(), right.begin(), right.end());
